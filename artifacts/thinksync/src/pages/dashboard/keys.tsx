...
export default function DashboardKeysPage() {
  ...
  function onGenerate(event: FormEvent<HTMLFormElement>) {
    ...
    generateMutation.mutate(
      ...
      { onSuccess: (result) => setLatestRawKey(result.raw_key) }
    );
  }
      ...
      <Card>
        ...
        <CardContent>
          <form className="grid gap-3 md:grid-cols-[1fr_180px_auto]" onSubmit={onGenerate}>
            ...
            <Input type="number" min={1} max={365} value={expiresInDays} onChange={(e) => setExpiresInDays(e.target.value ? Number(e.target.value) : "")} placeholder="Expiry (days)" />
              ...
              {data?.map((key) => (
                <TableRow key={key.id}>
                  ...
                  <TableCell className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => rotateMutation.mutate(key.id)}>Rotate</Button>
                    <Button size="sm" variant="destructive" onClick={() => revokeMutation.mutate(key.id)}>Revoke</Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
