#!/usr/bin/env python3
"""
Generate a complete, production-ready admin-bot.ts with menu-driven UX.
Run: python3 scripts/generate-admin-bot.ts.py > artifacts/api-server/src/bot/admin-bot.ts
"""
import sys
...
with open("/root/ThinkSync-Models/artifacts/api-server/src/bot/admin-bot.ts", "w") as f:
    f.write(content)

print(f"Done! Written {len(content)} chars")
print("Now run: cd /root/ThinkSync-Models/artifacts/api-server && pnpm build")
